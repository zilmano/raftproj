...
func TestReElection2A(t *testing.T) {
    ...
    fmt.Printf("1: cfg.checkOneLeader()")
	leader1 := cfg.checkOneLeader()
    fmt.Printf("1: done")

	// if the leader disconnects, a new one should be elected.
    fmt.Printf("2: if the leader disconnects, a new one should be elected.")
    ...
    fmt.Printf("3: if the old leader rejoins, that shouldn't, disturb the new leader.")
	cfg.connect(leader1)
	...
	// be elected.
    fmt.Printf("4: if there's no quorum, no leader should be elected")
    ...
    fmt.Printf("5:  if a quorum arises, it should elect a leader.")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
    fmt.Printf("5: done")

	// re-join of last node shouldn't prevent leader from existing.
    fmt.Printf("6: re-join of last node shouldn't prevent leader from existing.")
	cfg.connect(leader2)
	cfg.checkOneLeader()
    fmt.Printf("6: done")
